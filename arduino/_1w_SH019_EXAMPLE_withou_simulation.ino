...
void familyDE_red_card_code(){
  ...
  if ( data[10]==OneWire::crc8(data, 10) ) { // compare CRC
    ...
    if (new_card_code==true) {
      ...
      Serial.println("");
      if (true) { // examle inform person by beep for anable acces (only example, must result your own acces system)
...
void ini_card_reader(void) {
  ...
  delayMicroseconds(500); // for send debug information if no debug then no need
  set_simulation_DS1990(0); // this unset simulatin, other valu then 0 start simulation
...
void setup(void) {
  ...
  for (;;){ // find right w1 device
    ...
    if (addr[0]==0xDE) break;
    Serial.println("Device is not a RDM6300");
  }
  Serial.println("Device is a RDM6300");
  ini_card_reader();
}
...
void loop(void) {
  ...
  */
  ...
  Serial.println();
  if (addr[0]!=0xDE){
    Serial.println("No Chip RDM6300");
    return;
  }
  Serial.println("Is Chip RDM6300");
  //set_beep(2000, 100);
  ...
  //set_status_byte(1, 1);
  //delay(2000);
  set_status_byte(0, RED_LED_MASK);
  delay(150);
  set_status_byte(0, GREEN_LED_MASK);
  delay(1000);
  set_status_byte(1, RED_LED_MASK);
  delay(150);
  set_status_byte(1, GREEN_LED_MASK);
